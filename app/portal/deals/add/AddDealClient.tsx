...
import { createDealWithItems } from '@/lib/actions';
...
export default function AddDealClient({ initialCategories, initialMenuItems }: AddDealClientProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await createDealWithItems({
        name: formData.name,
        description: formData.description || `Includes: ${formData.items.map(i => i.name).join(', ')}`,
        ...
        usage_limit: formData.max_uses > 0 ? formData.max_uses : undefined,
      ...
      toast.success(`Deal created successfully!${result.data?.code ? ` Code: ${result.data.code}` : ''}`);

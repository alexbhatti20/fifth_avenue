...
function OrderTimer({ 
  ...
  useEffect(() => {
    const startTime = kitchenStartedAt ? new Date(kitchenStartedAt) : new Date(createdAt);
    const interval = setInterval(() => {
      const diff = Math.floor((Date.now() - startTime.getTime()) / 1000);
      setElapsed(diff);
    }, 1000);
    return () => clearInterval(interval);
  }, [createdAt, kitchenStartedAt]);
...
// Kitchen Order Card - Premium White Card with Animated Red Gradient
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        animate={{
          background: [
            'linear-gradient(135deg, #ef4444 0%, #f43f5e 50%, #f97316 100%)',
            'linear-gradient(135deg, #f97316 0%, #ef4444 50%, #f43f5e 100%)',
            'linear-gradient(135deg, #f43f5e 0%, #f97316 50%, #ef4444 100%)',
            'linear-gradient(135deg, #ef4444 0%, #f43f5e 50%, #f97316 100%)',
          ],
        }}
        transition={{
      ...
      <Card className="relative overflow-hidden bg-white dark:bg-zinc-900 m-1 rounded-xl shadow-lg">
        ...
        <div className="px-4 py-3 bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-950/30 dark:to-orange-950/30 border-b">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              <motion.div 
                className="min-w-[56px] h-12 px-2 rounded-xl bg-gradient-to-br from-red-500 to-orange-500 flex flex-col items-center justify-center text-white shadow-lg shadow-red-500/30"
                whileHover={{ scale: 1.05, rotate: 2 }}
              >
                <span className="text-[9px] text-white/80 font-medium">ORD</span>
                <span className="text-xs font-bold truncate max-w-full">#{order.order_number}</span>
              </motion.div>
              <div>
...
export default function KitchenPage() {
  ...
  // Inject CSS styles for gradient animation
  useEffect(() => {
    const styleId = 'kitchen-gradient-styles';
  ...
  // Fetch orders using optimized RPC
  const fetchOrders = useCallback(async () => {
    try {
      // Try v2 RPC first
      const { data: rpcData, error: rpcError } = await supabase.rpc('get_kitchen_orders_v2');
      ...
      // Fetch stats - fallback to counting if RPC not available
      const { data: statsData, error: statsError } = await supabase.rpc('get_kitchen_stats');
  ...
  const handleStatusChange = async (orderId: string, newStatus: string) => {
    ...
    try {
      // Try v2 RPC first (non-blocking)
      const { error: rpcError } = await supabase.rpc('update_kitchen_order_status', {
        p_order_id: orderId,
        p_status: newStatus
      });

...
import { 
  getCustomerDetailServer, 
  banCustomerServer, 
  unbanCustomerServer,
  refreshCustomersServer,
  refreshCustomerStatsServer
} from '@/lib/actions';
...
import type { CustomerServer, CustomerStatsServer } from '@/lib/server-queries';
...
interface CustomersClientProps {
  initialCustomers: CustomerServer[];
  initialStats: CustomerStatsServer | null;
}
...
function BanUnbanSection({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === 'ban') {
        result = await banCustomerServer({
        ...
        result = await unbanCustomerServer({
          customerId: customer.customer_id,
          unbannedBy: adminId,
          sendEmail,
          email: customer.customer_email,
      ...
      toast.success(`${customer.customer_name} has been ${mode === 'ban' ? 'banned' : 'unbanned'}${sendEmail ? ' and email sent' : ''}`);
      onSuccess();
...
function CustomerDetailDialog({
  ...
  useEffect(() => {
    async function fetchCustomerDetail() {
      ...
      // Avoid blocking UI - set loading first
      setIsLoading(true);
      setCustomer(null); // Clear previous customer immediately

      try {
        // Use server action instead of client RPC
        const result = await getCustomerDetailServer(customerId);
...
// Main Customer Management Page Client Component
export default function CustomersClient({ initialCustomers, initialStats }: CustomersClientProps) {
  const router = useRouter();
  const { employee: currentEmployee, role } = usePortalAuth();
  
  // Data state - use SSR data as initial
  const [customers, setCustomers] = useState<Customer[]>(initialCustomers as Customer[]);
  const [stats, setStats] = useState<CustomerStats | null>(initialStats as CustomerStats | null);
  const [isLoading, setIsLoading] = useState(false);
  ...
  // FIX: Wrapped in useCallback for proper memoization
  const fetchCustomers = useCallback(async (search?: string, filter?: string) => {
    if (!isAuthorized) return;
    
    setIsLoading(true);
    try {
      const result = await refreshCustomersServer({
        search,
        filter,
        limit: 100,
        offset: 0,
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const result = await refreshCustomerStatsServer();
      if (result.success) {
  ...
  // Load data when filters change (only refetch when search/filter changes from initial)
  useEffect(() => {
    if (isAuthorized && (debouncedSearch || statusFilter !== 'all')) {

...
export default function BillingPage() {
  ...
  const [pendingOrders, setPendingOrders] = useState<BillableOrder[]>([]);
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase.rpc('get_billing_dashboard_stats');
  ...
  // Fetch pending orders for dashboard preview using optimized RPC
  const fetchPendingOrders = useCallback(async () => {
    setIsLoadingPending(true);
    try {
      const { data, error } = await supabase.rpc('get_billing_pending_orders', {
        p_limit: 5, // Show only first 5 on dashboard
      });
    ...
    <div className="space-y-4 md:space-y-6 pb-8">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="pending" className="mt-0">
          <PendingOrdersList onSelectOrder={handleGenerateBill} />
        </TabsContent>

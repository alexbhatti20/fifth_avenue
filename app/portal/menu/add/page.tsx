...
export default function AddMenuItemPage() {
  ...
  const fetchCategories = async () => {
    try {
      const { data } = await supabase.from('menu_categories').select('id, name, slug').order('display_order');
      setCategories(data || []);
    ...
    if (enabled && formData.size_variants.length === 0) {
      ...
      setFormData(prev => ({
        ...
        size_variants: DEFAULT_SIZES.map((size, i) => ({
          size,
          price: prev.price + (i * 50), // Increment price for each size
          is_available: true
        }))
      }));
    } else {
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      // Use RPC function to create menu item with size variants
      const { data, error } = await supabase.rpc('create_menu_item_advanced', {
            ...
            >
              <CardContent className="p-4 flex items-center gap-4">
                <div className={cn(
                  ...
                  formData.has_variants ? 'bg-orange-500 text-white' : 'bg-zinc-200 dark:bg-zinc-700'
                )}>
                  <Ruler className="h-6 w-6" />
                </div>
                <div className="flex-1">
                  <h4 className="font-medium">Enable Size Variants</h4>
                  <p className="text-sm text-muted-foreground">Offer this item in multiple sizes (Small, Medium, Large, etc.)</p>
                </div>
                {formData.has_variants && <Check className="h-5 w-5 text-orange-500" />}
              </CardContent>
            </Card>
                ...
                {formData.size_variants.length === 0 ? (
                  <Card className="border-dashed">
                    <CardContent className="py-8 text-center">
                      ...
                      <Button variant="link" onClick={addSizeVariant}>Add your first size</Button>
                    </CardContent>
                  </Card>
                ) : (
                  ...
                  <Card className="bg-blue-500/10 border-blue-500/20">
                    <CardContent className="pt-4">
                      <p className="text-sm text-blue-600">
                        <strong>Tip:</strong> The base price set in Step 2 will be used as default. Size-specific prices above will override it on the menu.
                      </p>
                    </CardContent>
                  </Card>
                )}
            ...
            <Card className="bg-zinc-50 dark:bg-zinc-800/50">
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {formData.has_variants && formData.size_variants.length > 0
                      ? `Rs. ${Math.min(...formData.size_variants.map(v => v.price))} - ${Math.max(...formData.size_variants.map(v => v.price))}`
                      : `Rs. ${formData.price}`
                    }
                  </span>
                </div>
                ...
                <div className="flex gap-2 flex-wrap justify-end">
                  ...
                  {formData.has_variants && <Badge variant="outline" className="bg-orange-500/10">Has Sizes</Badge>}
                </div>
              </CardContent>
            </Card>
          </motion.div>

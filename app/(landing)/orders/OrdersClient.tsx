...
export default function OrdersClient({ initialOrders }: OrdersClientProps) {
  ...
  useEffect(() => {
    // Give time for localStorage to be checked
  ...
  useEffect(() => {
    // Only redirect after we've properly checked auth
    if (hasCheckedAuth && !authLoading && !user) {
      router.push("/auth");
    }
  }, [user, authLoading, router, hasCheckedAuth]);

  // Server-side data is already provided via SSR - no client fetch needed
  // This removes the visible API call from the Network tab

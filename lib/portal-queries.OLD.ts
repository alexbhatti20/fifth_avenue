...
async function isAdminOrManager(): Promise<boolean> {
  if (!isSupabaseConfigured) return false;
  try {
    // First check Supabase session
    const { data: { session } } = await supabase.auth.getSession();
    ...
    // Fallback: Check localStorage for portal auth
    if (typeof window !== 'undefined') {
      const userData = localStorage.getItem('user_data');
      const userType = localStorage.getItem('user_type');
      if (userData && userType) {
        try {
          ...
          const role = parsed.role || userType;
...
export async function getCurrentEmployee(): Promise<Employee | null> {
  ...
  // Only log in development and if there's an actual error (not just missing session)
  if (process.env.NODE_ENV === 'development' && authError && authError.message !== 'Auth session missing!') {
  }
  
  if (!user) {
    // No Supabase session - PortalProvider will fallback to localStorage
    return null;
  }

  // Use secure RPC instead of direct table access
  const { data, error } = await supabase.rpc('get_employee_by_auth_user', {
...
export async function updateEmployee(employeeId: string, updates: Partial<Employee>): Promise<{ success: boolean; error?: string }> {
  ...
  const { data, error } = await supabase.rpc('update_employee_complete', {
...
export async function getInvoices(filters?: {
  ...
  const { data, error } = await supabase.rpc('get_recent_invoices', {
    p_start_date: filters?.startDate || null,
    p_end_date: filters?.endDate || null,
    p_payment_method: filters?.status || null, // Note: RPC uses payment_method, not status
    p_limit: filters?.limit || 50,
  });
...
export async function getAttendanceRecords(filters?: {
  ...
  // Parse year/month from startDate if provided, otherwise use current
  ...
  // The RPC returns { success, month, attendance } format
...
export async function getWebsiteContent(): Promise<WebsiteContent[]> {
  ...
  // Use secure RPC - get_website_settings_internal for website_content table
  const { data, error } = await supabase.rpc('get_website_settings_internal');

  if (error) {
    console.error('getWebsiteContent error:', error);
    return [];
  }

  // The RPC returns { success, settings } format
...
export async function updateWebsiteContent(
  ...
  // Use secure RPC instead of direct table access
  const { data, error } = await supabase.rpc('upsert_website_settings_internal', {
    p_settings: content,
  });

  if (error) {
    console.error('updateWebsiteContent error:', error);
    return { success: false, error: error.message };
  }
...
export async function getPromoCodes(): Promise<PromoCode[]> {
  ...
  // The RPC returns { success, promos, total, limit, offset } format
  if (data?.success && data?.promos) {
    return data.promos as PromoCode[];
  }
...
export async function createPromoCode(
  ...
  // Note: promo_codes table has RLS with policies that allow employees to insert
  // The insert policy 'promo_codes_insert_employees' allows inserts
...
export async function updateReviewVisibility(
  ...
  // Use employee-based RPC if employeeId provided (bypasses auth.uid() issues)
  if (employeeId) {
    const { data, error } = await supabase.rpc('update_review_visibility_by_employee', {
...
export async function getAdminReviewsAdvanced(
  ...
  // If employeeId is provided, use the new RPC that bypasses auth.uid() checks
  // This is needed because JWT-based auth doesn't set auth.uid() in Supabase
  if (employeeId) {
    const { data, error } = await supabase.rpc('get_admin_reviews_by_employee', {
      p_employee_id: employeeId,
      p_status: filters?.status || null,
    ...
    if (error) {
      console.error('getAdminReviewsAdvanced error:', error);
      return { success: false, error: error.message, reviews: [], total_count: 0, has_more: false };
    }

    // The new RPC returns { success, reviews, stats } format
    if (data && data.success) {
      return {
        success: true,
        ...
        has_more: false,
        stats: data.stats, // Include stats in the response
      } as AdminReviewsResponse & { stats?: any };
    }

    return data || { success: false, error: 'Unknown error', reviews: [], total_count: 0, has_more: false };
  }

  // Fallback to original RPC (for cases where auth.uid() works)
...
export async function bulkUpdateReviewVisibility(
  ...
  if (employeeId) {
    const { data, error } = await supabase.rpc('bulk_update_review_visibility_by_employee', {
...
export async function replyToReviewAdvanced(
  ...
  // Use employee-based RPC if employeeId provided (bypasses auth.uid() issues)
  if (employeeId) {
    const { data, error } = await supabase.rpc('reply_to_review_by_employee', {
      p_review_id: reviewId,
      p_reply: reply,
      p_employee_id: employeeId,
    });
...
export async function deleteReviewAdvanced(
  ...
  // Use employee-based RPC if employeeId provided (bypasses auth.uid() issues)
  if (employeeId) {
    const { data, error } = await supabase.rpc('delete_review_by_employee', {
      p_review_id: reviewId,
      p_employee_id: employeeId,
    });
